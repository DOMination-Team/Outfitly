      ...
      <div className="max-w-7xl mx-auto relative z-10">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-6 gap-12 mb-16">
          ...
          <div className="lg:col-span-2">
            ...
            >
              ...
              <p className="opacity-70 mb-6 leading-relaxed" style={{ color: colors.textPrimary }}>
                Your smart wardrobe companion. Discover, create, and share amazing outfits with the power of AI and community.
              </p>

              {/* Contact Info */}
              <div className="space-y-3">
                {[{ icon: Mail, text: "hello@outfitly.com" }, { icon: Phone, text: "+1 (555) 123-4567" }, { icon: MapPin, text: "San Francisco, CA" }].map((info, i) => (
                  <div key={i} className="flex items-center gap-3 opacity-70 hover:opacity-100 transition-opacity">
                    <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-[var(--outfitly-primary)] to-[var(--outfitly-primary-hover)] flex items-center justify-center">
                      <info.icon className="w-4 h-4" style={{ color: "var(--outfitly-text-light)" }} />
                    </div>
                    <span className="text-sm" style={{ color: colors.textPrimary }}>{info.text}</span>
                  </div>
                ))}
              </div>
            </motion.div>
          ...
          {Object.entries(footerLinks).map(([category, links], idx) => (
            ...
            >
              <h4 className="mb-4" style={{ color: colors.textPrimary }}>{category}</h4>
        ...
        >
          ...
          <div className="relative z-10 p-12 text-center">
            <h3 className="text-3xl mb-4" style={{ color: "var(--outfitly-text-light)" }}>Stay in Style</h3>
            <p className="mb-8 opacity-90 max-w-xl mx-auto" style={{ color: "var(--outfitly-text-light)" }}>
