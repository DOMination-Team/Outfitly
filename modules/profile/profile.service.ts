...
export const getUserOutfitsPaginated = async (
  userId: string,
  query: IPaginationQuery,
) => {
  const { page, limit, order = "desc", field = "createdAt" } = zodValidation(profileListQuerySchema, query);
  return findUserOutfits(userId, { page, limit }, order, field);
};

// Get liked outfits paginated
export const getLikedOutfitsPaginated = async (
  userId: string,
  query: IPaginationQuery,
) => {
  const { page, limit, order = "desc", field = "createdAt" } = zodValidation(profileListQuerySchema, query);
...
export const getLikedProductsPaginated = async (
  userId: string,
  query: IPaginationQuery,
) => {
